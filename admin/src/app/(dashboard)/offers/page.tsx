...
export default function OffersPage() {
  ...
  const getOfferStatus = (offer: Offer) => {
    ...
    if (!offer.is_active) {
      return { label: 'معطل', color: 'bg-gray-100 text-gray-600', icon: EyeOff };
    }
    if (isPast(endDate)) {
      return { label: 'منتهي', color: 'bg-red-100 text-red-600', icon: Clock };
    }
    if (isFuture(startDate)) {
      return { label: 'قادم', color: 'bg-blue-100 text-blue-600', icon: Calendar };
    }
    return { label: 'نشط', color: 'bg-green-100 text-green-600', icon: Tag };
  };
